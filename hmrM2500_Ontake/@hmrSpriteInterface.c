...
#define HMR_SPRITEINTERFACE_C_INC 106
#
#define HMLIB_NOLIB
...
//画像サイズの指定(定義した３通り)
static unsigned char _spriteitf_varCmd_SET_IMAGE_SIZE[9]={0x56,0x00,0x31,0x05,0x04,0x01,0x00,0x19,0xFF};
spriteitf_command _spriteitf_varCmd_SetImageSize(spriteitf_imagesize_t PictureSize_){
	spriteitf_command Ans;
	_spriteitf_varCmd_SET_IMAGE_SIZE[8]=PictureSize_;
...
//強制リセットで利用中か
boolian x_spriteitf_timer_PowerReset=hmLib_false;
//timer機能の初期化
void _spriteitf_timer_initialize(void){
	x_spriteitf_timer_fnTimeOut=0;
	x_spriteitf_timer_PowerReset=hmLib_false;
...
//写真データ用バッファに書き込んだデータ数要求
//spriteitf_dsize_t spriteitf_getBufWrireNum(void){return _spriteitf_BufWriteNum;}
//写真基礎データ受信完了フラグをクリア
...
void _spriteitf_fnFinTakePicCommand(void){
	...
	{
	case spriteitf_Status_SetImageSize:
		_spriteitf_Status=spriteitf_Status_ActiveImageSize;
		return _spriteitf_uart_sendCommand(_spriteitf_Cmd_ResetCmr,_spriteitf_fnFinTakePicCommand,0);
	case spriteitf_Status_ActiveImageSize:
...
void _spriteitf_fnPowerReset(void){
	...
	if(count > (unsigned char)(5000/spriteitf_RecvTimeOut + 1)){
		...
		_spriteitf_power(1);			//電源入れる 初期化前に電源を入れておく
		_spriteitf_uart_fget_interrupt_disable();
		_spriteitf_uart_fput_interrupt_disable();
...
boolian spriteitf_PowerReset(void){
	...
	_spriteitf_uart_fget_interrupt_disable();
	_spriteitf_uart_fput_interrupt_disable();
	//タイマー割り込みを無効化（電源OFFなので、通信タイムアウトもろとも消す。）
	_spriteitf_timer_stop();
	//カメラ電源OFF
	_spriteitf_power(0);
	//カメラとのuart無効化これで完全に電源をOFFにする　これをやらないと１．５Vの電圧が送信ピンからやってきて残ってしまう
	//uart終端化
