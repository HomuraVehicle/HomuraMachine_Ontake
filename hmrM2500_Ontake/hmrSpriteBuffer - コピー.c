...
void spritebuf_share_withMesasge(spritebuf_id ID,cstring* pStr,uint16 PicDataPos,
	...
	cstring_construct_static(pStr,x_spritebuf_Buffer[ID-1].Buf,x_spritebuf_Buffer[ID-1].Buf+x_spritebuf_Buffer[ID-1].DataSize);
...
//写真データ書き込み領域を取得する
unsigned char* spritebuf_getBufPtr(spritebuf_id ID){
	//アクセス可能な領域でなければ無視
...
//写真データサイズ書き込み領域を取得する
uint16* spritebuf_getBufCnt(spritebuf_id ID){
	//アクセス可能な領域でなければ無視
	if(ID<1 || spritebuf_Num<=ID)return 0;
