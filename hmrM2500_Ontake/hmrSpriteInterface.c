...
//=====================timer用define========================//
#define _spriteitf_timer_clear()					timerCMR_reset()
#define _spriteitf_timer_interrupt					timerCMR_interrupt
#define _spriteitf_timer_interrupt_disable()		devmng_interrupt_disable_timerCMR_interrupt()//timer1_disable_interrupt()//
#define _spriteitf_timer_interrupt_enable()			devmng_interrupt_enable_timerCMR_interrupt()//timer1_enable_interrupt()//
#define _spriteitf_timer_interrupt_clear()			timerCMR_clear_interrupt_flag()
#define _spriteitf_timer_interrupt_initialize(Time)	timerCMR_initialize(Time)
//==========================================================//

//=====================uart用define=========================//
#define x_spriteitf_uart_initialize(BaudRate)				hmr_uart2_initialize(BaudRate)
#define _spriteitf_uart_fputc(c)					rs_fputc(c,Stream_CMR)
#define _spriteitf_uart_fput_interrupt				streamCMR_fput_interrupt//uart2_fput_interrupt//
#define _spriteitf_uart_fput_interrupt_set()		streamCMR_set_fput_interrupt_flag()//set_interrupt_fputflag(Stream_CMR)//
#define _spriteitf_uart_fput_interrupt_clear()		clear_interrupt_fputflag(Stream_CMR)//streamCMR_clear_fput_interrupt_flag()//
#define _spriteitf_uart_fput_interrupt_enable()		devmng_interrupt_enable_streamCMR_fput_interrupt()//enable_interrupt_fput(Stream_CMR)//
#define _spriteitf_uart_fput_interrupt_disable()	devmng_interrupt_disable_streamCMR_fput_interrupt()//disable_interrupt_fput(Stream_CMR)//
...
#define _spriteitf_uart_fget_interrupt				streamCMR_fget_interrupt//uart2_fget_interrupt//
#define _spriteitf_uart_fget_interrupt_set()		streamCMR_set_fget_interrupt_flag()	//set_interrupt_fgetflag(Stream_CMR)//
#define _spriteitf_uart_fget_interrupt_clear()		streamCMR_clear_fget_interrupt_flag()//clear_interrupt_fgetflag(Stream_CMR)//
#define _spriteitf_uart_fget_interrupt_disable()	devmng_interrupt_disable_streamCMR_fget_interrupt()//disable_interrupt_fget(Stream_CMR)//
#define _spriteitf_uart_fget_interrupt_enable()		devmng_interrupt_enable_streamCMR_fget_interrupt()//enable_interrupt_fget(Stream_CMR)//
//==========================================================//
...
void _spriteitf_uart_initialize(void){
	...
	//uart module初期化
	x_spriteitf_uart_initialize(38400);
	//割り込み予約
...
//パワーリセット開始時に呼ぶユーザー定義関数
spriteitf_vFp_v _spriteitf_fnInformStrtPwReset=0;
//パワーリセット終了時に呼ぶユーザー定義関数
spriteitf_vFp_v _spriteitf_fnInformFinPwReset=0;
...
void _spriteitf_fnPowerReset(void){
	...
	spriteitf_vFp_v fnInformStrtPwReset=0;
	spriteitf_vFp_v fnInformFinPwReset=0;
	...
	if(count > (unsigned char)(5000/spriteitf_RecvTimeOut + 1)){
		...
		_spriteitf_Status=spriteitf_Status_Idle;
		//パワーリセット完了通知関数実行
		if(_spriteitf_fnInformFinPwReset)_spriteitf_fnInformFinPwReset();
		...
		fnInformStrtPwReset=_spriteitf_fnInformStrtPwReset;
		fnInformFinPwReset=_spriteitf_fnInformFinPwReset;

		//spriteitf初期化
		spriteitf_initialize();
		//退避させていたフラグ回収
		_spriteitf_AutoResetFlag=tmpAutoPwResetFlag;
		_spriteitf_fnInformStrtPwReset=fnInformStrtPwReset;
		_spriteitf_fnInformFinPwReset=fnInformFinPwReset;
...
boolian spriteitf_PowerReset(void){
	...
	spriteitf_vFp_v fnInformStrtPwReset=_spriteitf_fnInformStrtPwReset;
	spriteitf_vFp_v fnInformFinPwReset=_spriteitf_fnInformFinPwReset;

	//パワーリセット開始通知関数実行
	if(_spriteitf_fnInformStrtPwReset)_spriteitf_fnInformStrtPwReset();
	//spriteitf終端化
	spriteitf_finalize();
	//退避させていたフラグ回収
	_spriteitf_AutoResetFlag=tmpAutoPwResetFlag;
...
//パワーリセット開始・終了時のInform関数登録
void spriteitf_resistInformPwReset(spriteitf_vFp_v fnInformStrtPwReset,spriteitf_vFp_v fnInformFinPwReset){
	_spriteitf_fnInformStrtPwReset=fnInformStrtPwReset;
	_spriteitf_fnInformFinPwReset=fnInformFinPwReset;
}
...
//	_spriteitf_power(1);
	...
	_spriteitf_fnInformStrtPwReset=0;
	_spriteitf_fnInformFinPwReset=0;
	//エラー処理登録
