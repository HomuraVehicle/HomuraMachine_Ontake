...
#endif
//*******************************//
typedef sint16(*task_s16Fp_s16)(sint16);
typedef void(*task_vFp_v)(void);
typedef task_s16Fp_s16 task_function;
typedef task_vFp_v task_inform;
//タスククラス
typedef struct{
	//タスク処理で実行したい関数
	task_function Fp;
	//タスクが強制停止された場合の通知関数
	task_inform InformStop;
	//タスクの実行間隔
	uint16 Interval;
}task;
typedef struct{
	...
	task_vFp_v Fp_informStart;
	//タスクシステムからすべての登録が消えたと場合に通知
	task_vFp_v Fp_informStop;
	//タスク用関数配列
	task* Buf;
	//最大のタスクの数
	uint8 MaxSize;
...
//タスクシステムの登録がゼロでなくなった場合に通知
void task_resgist_informStart(taskmaster* TaskMaster,task_vFp_v Fp);
//タスクシステムからすべての登録が消えたと場合に通知
void task_resgist_informStop(taskmaster* TaskMaster,task_vFp_v Fp);
#
#endif
